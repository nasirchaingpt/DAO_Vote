...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from12, paymaster, paymasterInput } = transaction;
  ...
  if (from12 && !isAddress(from12))
    throw new InvalidAddressError({ address: from12 });
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce, to, from: from12, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce, to, from: from12, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from12),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from12, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from12,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from12, to } = transaction;
  if (from12 && !isAddress(from12))
    throw new InvalidAddressError({ address: from12 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
function assert4(envelope) {
  ...
  if (type2 === "p256") {
    ...
    const missing = [];
    if (typeof ((_a = p2562.signature) == null ? void 0 : _a.r) !== "bigint")
...
function deserialize(value) {
  ...
  if (size2 === 65) {
    const signature = fromHex2(serialized);
    assert3(signature);
...
function verify4(signature, parameters) {
  ...
  if (!address)
    return false;
  const envelope = from5(signature);
  if (envelope.type === "secp256k1") {
    if (!address)
      return false;
    return verify({
...
function hash(authorization, options = {}) {
  ...
  return keccak2562(concat("0x05", fromHex(toTuple2(presign ? {
...
function from7(authorization, options = {}) {
  ...
  if (options.signature)
    return {
      ...resolved,
      signature: from5(options.signature)
    };
  return resolved;
}
...
function from8(tokenIdOrAddress) {
...
function compute(value) {
  const hash4 = keccak2562(encode(from3("address, bytes32"), [
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload3(from10(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
...
function from10(envelope, options = {}) {
  ...
  return {
    ...envelope_,
    ...signature ? { signature: from5(signature) } : {},
...
function serialize4(envelope, options = {}) {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload3(from10(envelope)),
  ...
  const serialized = [
    ...
    ...signature ? [serialize(from5(signature))] : []
  ];
  return concat(options.format === "feePayer" ? feePayerMagic : serializedType, fromHex(serialized));
}
...
(function(authorize2) {
  async function inner(action, client, parameters) {
    var _a;
    const { accessKey, chainId = (_a = client.chain) == null ? void 0 : _a.id, expiry, limits, scopes, ...rest } = parameters;
...
async function getRemainingLimit(client, parameters) {
  ...
  if (!account_)
    throw new Error("account is required.");
  const account = parseAccount(account_);
  const hardfork = (_a = client.chain) == null ? void 0 : _a.hardfork;
...
var chainConfig5 = {
  ...
  prepareTransactionRequest: [
    async (r, { phase }) => {
      ...
      if (phase === "afterFillParameters") {
        if (request.feePayer) {
          if (((_a = request.keyAuthorization) == null ? void 0 : _a.signature.type) === "webAuthn")
      ...
      const useExpiringNonce = await (async () => {
        ...
        if (request.feePayer && typeof request.nonceKey === "undefined")
          return true;
        const address = (_a2 = request.account) == null ? void 0 : _a2.address;
  ...
  serializers: {
    ...
    transaction: ((transaction, signature) => serialize5(transaction, signature))
  },
  async verifyHash(client, parameters) {
    const { address, hash: hash4, signature, mode: mode2 } = parameters;
    const envelope = (() => {
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode2 === "allowAccessKey") {
        ...
        if (keyInfo.expiry <= BigInt(Math.floor(Date.now() / 1e3)))
          return false;
        const innerPayload = envelope.version === "v2" ? keccak256(concat("0x04", hash4, address)) : hash4;
...
//# sourceMappingURL=chunk-A3ZQWUTT.js.map
